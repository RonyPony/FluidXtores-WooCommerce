...
import 'package:fluidxtores/contracts/user_service_contract.dart';
...
import 'package:fluidxtores/models/users_response.dart';
...
class UserService implements UserServiceContract {
  ...
  Future<bool> updateClientUserProfileImage(
  ...
  Future<bool> updateDriverUserProfileImage(
      FormData data, UserResponse user) async {
    String? errorMessage = '';
    try {
      final client = NetworkUtil.getTokenClient();
      ...
      final response =
          await client.patch('drivers/${user.id}/avatar', data: data);
      if (response.statusCode! < 400) {
        return true;
      }
